...
### 2. ONNX Runtime
**Current:**
- requirements.txt: onnxruntime-gpu==1.17.1
- requirements-mac.txt: onnxruntime==1.17.1

**Status:** ✅ CORRECT
- Different packages for different platforms (intended)
- Version 1.17.1 is consistent
...
Using latest LTS versions that work across all platforms:

```
PyTorch: 2.4.0 (latest stable, supports torch 2.4)
TorchVision: 0.19.0 (matches torch 2.4)
TorchAudio: 2.4.0 (matches torch 2.4)
ONNX Runtime: 1.18.1 (latest stable)
...
- PyTorch: 2.4.0 → 2.4.0 (latest stable)
- TorchVision: 0.17.0 → 0.19.0 (matches torch 2.4)
- TorchAudio: 2.2.0 → 2.4.0 (matches torch 2.4)
- ONNX Runtime: 1.17.1 → 1.18.1 (latest)
- Transformers: 4.44.0 → 4.41.2 (more stable)
...
- ONNX Runtime GPU: 1.17.1 → 1.18.1 (latest)
- Transformers: 4.44.0 → 4.41.2 (more stable)
...
1. **2.4.0 is latest stable PyTorch** with good Metal support on Mac
2. **cu124** is latest CUDA support (backward compatible with cu121)
3. **1.18.1 ONNX** fixes several compatibility issues from 1.17.1
4. **4.41.2 Transformers** is more stable than 4.44.0 (which is very new)
5. **numpy 1.26.4** stays pinned (works with all modern packages)
6. **3.0.1 Sentence-Transformers** explicitly requires transformers 4.41.2+
